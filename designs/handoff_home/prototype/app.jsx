...
function App() {
  ...
  if (scene === "splash") {
    ...
    content = <Home force={homeForce} homeDir={t.homeDir} errKind={errKind} onOpenSettings={openSetup} />;
    if (t.homeDir === "single" || homeForce === "error") {
      ch = { statusDark: true, navDark: true, statusBg: INK, navBg: INK, frameBg: INK };
    } else if (t.homeDir === "cards") {
      // light surface — dark icons top and bottom
      ch = { statusDark: false, navDark: false, statusBg: T.sunken, navBg: T.sunken, frameBg: T.sunken };
    } else {
  ...
  return (
    <div style={{ position: "relative", height: "100vh", width: "100vw", overflow: "hidden", background: "radial-gradient(120% 120% at 50% 0%, #23262d 0%, #15171b 60%, #0e0f12 100%)", fontFamily: T.brand }}>
      ...
      <TweaksPanel>
        ...
        <TweakRadio label="Direction" value={t.homeDir} options={["cards", "equal", "dominant", "single"]} onChange={(v) => setTweak("homeDir", v)} />
        <div style={{ fontSize: 11, color: "#8A929E", margin: "2px 0 10px", lineHeight: 1.5 }}>cards = floating buttons + gap · equal = 50/50 panels · dominant = Describir bigger · single = one voice button</div>
...
function Caption({ scene }) {
  const map = {
    ...
    "home-idle": "Two big buttons anchored low for thumb reach. Tap one — the cycle plays inside that button; the other dims.",
